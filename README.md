...
매주 발행 기준으로 트랙 A를 우선 소진하되, 그 주에 시의성 있는 규제 뉴스가 없으면 트랙 B에서 소재를 고른다. 트랙 B 글도 "일반론 설명"이 아니라 실제 AWS 서비스명·오픈소스 프로젝트명·최근 버전 등 구체적 근거를 갖춰 써야 한다 — 소재가 항상 구체적이어야 "양산형 AI 콘텐츠"로 분류되지 않는다. 트랙 A는 배경/규제내용/체크리스트/참고자료 구조를, 트랙 B는 배경/아키텍처 옵션 비교·트레이드오프/체크리스트/참고자료 구조를 따른다.
