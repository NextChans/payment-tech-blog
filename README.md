...
### 트랙 A 확장 — 전자금융업 등록 시리즈 (여러 편으로 나눠 진행, 한 번에 한 편만)

시의성 있는 새 뉴스가 없는 주에는, 위 트랙 A 후보보다 먼저 이 시리즈에서 아직 안 쓴 편을 하나 골라 쓴다. 뉴스 기반이 아니라 "그래서 어떻게 등록하는데" 하고 검색하는 사람을 겨냥한 평가 참고형 시리즈다 — 회차별로 실제 조문·요건 수치는 작성 시점에 다시 확인한다(면제 기준 등은 이미 다른 글에서 다룬 수치와 어긋나지 않는지 확인).
