...
서울 안에는 물리적으로 떨어진 4개의 데이터센터 구역(AZ)이 있다. 같은 서울 안에서만 서버를 이중화해도 방식에 따라 차이가 크다. 전통적인 방식(Multi-AZ 인스턴스 배포)은 장애가 났을 때 다른 서버로 넘어가는(failover) 데 보통 60~120초가 걸리는데, 예비 서버를 여러 개 동시에 대기시켜두는 더 최신 방식(Multi-AZ DB 클러스터 배포)은 이 시간이 35초 미만으로 줄어든다. 후자는 평소에 데이터를 조회하는 속도도 빨라지는 덤이 있지만, 지원하는 데이터베이스 종류와 버전에 제약이 있어서 기존에 쓰던 서비스를 그대로 옮기지 못할 수도 있다 — 새로 만드는 서비스라면 처음부터 이 방식으로 시작하는 걸 검토할 만하다.
