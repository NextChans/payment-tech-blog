...
전자금융감독규정 제14조의2(금융회사가 클라우드를 쓸 때 지켜야 할 조항)에 따른 클라우드 이용은 이렇게 진행된다: 어떤 업무를 클라우드로 옮길지 정하고 중요도를 평가 → 클라우드 회사(AWS 같은 곳, 전문 용어로 CSP)가 안전한지 평가 → 보안 조치를 마련 → 회사 내부 정보보호위원회가 심의·승인 → 계약 체결 → 실제로 쓰기 시작하고, 계약 후 3개월 안에 금감원에 알림(사후보고) → 클라우드 이용을 끝낼 때는 데이터를 안전하게 옮기거나 지우는 계획(출구전략)까지 이행. 이 중에서 서버를 어떻게 설계할지에 직접 영향을 주는 지점이 세 곳 있다.
